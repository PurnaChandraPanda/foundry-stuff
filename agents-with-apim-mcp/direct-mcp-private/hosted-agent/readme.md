...
Hosted Python -> outbound VNet -> private APIM MCP (tool execution)
```
...
The code loads `.env` next to [main.py](./src/private-apim-responses/main.py)
...
export AZD_ENV_NAME="apim-private-dev"

azd ai agent init \
  --src ./src/private-apim-responses \
  --agent-name private-apim-direct-hosted \
...
azd deploy private-apim-direct-hosted -e "$AZD_ENV_NAME"

azd ai agent doctor
azd ai agent show private-apim-direct-hosted
```

No VNet/network infrastructure is created by these scripts. With restricted
egress, follow the
[source deployment firewall requirements](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/deploy-hosted-agent-code#firewall-requirements-for-private-virtual-networks)
and permit remote dependency resolution or use the documented bundled approach.
...
This happens before calling APIM. It is not an APIM subscription-header error,
and granting a role to your user account or APIM's managed identity does not
fix the hosted agent's permission.

The built-in **Foundry User** role (formerly **Azure AI User**) includes this
data action. Assign it to the hosted agent identity at the **existing project
scope**, not at subscription or resource-group scope. It grants broader project
data access than connection-secret reading alone; use an approved custom role
instead if your organization requires narrower permissions. Do not grant the
runtime Owner or Contributor as a substitute for this data-plane permission.
...
# Use this new agent's identity, not the public-route agent's principal.
HOSTED_AGENT_PRINCIPAL_ID="46e21267-db93-49ac-8dfc-502ffc62b06c"
...
Allow time for RBAC propagation, otherwise restart the failed runtime or redeploy
...
```bash
azd ai agent invoke private-apim-direct-hosted "Call createsAChatCompletion once. Set the top-level api-version argument to v1. Put model gpt-4o, messages containing the user request 'Explain private endpoints in two sentences.', and max_completion_tokens 120 INSIDE the required AzureCreateChatCompletionRequest object. Do not put those body fields at the top level."
