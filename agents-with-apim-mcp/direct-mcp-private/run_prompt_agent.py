"""Run a pinned direct-MCP prompt agent, handling every round of MCP approvals."""
    ...
    for round_number in range(21):
        ...
        for item in response.output:
            if item.type == "mcp_list_tools" and item.error is not None:
                raise RuntimeError(f"MCP discovery failed: {item.error}")
            if item.type == "mcp_call":
                print(f"MCP call: {item.name}, server: {item.server_label}")
                if item.error is not None:
                    raise RuntimeError(f"MCP call failed: {item.error}")
