...
In **Git Bash on Windows**, manually run this once in each new terminal before
running the wrapper:
...
This prevents Git Bash from converting Azure resource IDs into filesystem paths.
The wrapper does not set this variable for you; it checks that it is set and
still converts the analyzer and report paths for Windows Python. Linux, macOS,
and WSL with Linux Python do not need this export.

Then run the analysis in the same terminal:

```bash
./reachability.sh \
  --account /subscriptions/6---------------------------------03/resourceGroups/rg-std2/providers/Microsoft.CognitiveServices/accounts/aifoundry3738 \
  --endpoint aifoundry3738storage.blob.core.windows.net --port 443
```
...
```bash
./reachability.sh \
  --project /subscriptions/6---------------------------------03/resourceGroups/rg-std22624/providers/Microsoft.CognitiveServices/accounts/aiservicesxee2/projects/projectxee2 \
  --endpoint myapim3252.azure-api.net --port 443
```
...
If using the Python entry point directly in Git Bash instead of the wrapper,
disable MSYS path conversion so Azure resource IDs are passed unchanged:

```bash
MSYS_NO_PATHCONV=1 python3 analyze.py --account "/subscriptions/<subscription-id>/resourceGroups/<resource-group>/providers/Microsoft.CognitiveServices/accounts/<account>" --endpoint api.contoso.com --port 443
```

Use `analyze.py`, not `.\analyze.py`, in Git Bash. The analyzer resolves Azure CLI
from `PATH`, including its `az.cmd` launcher on Windows. If it is not found,
install Azure CLI or fix `PATH`, then restart the terminal (and VS Code if used).
