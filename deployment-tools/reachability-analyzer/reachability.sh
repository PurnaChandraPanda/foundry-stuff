...
case "${MSYSTEM:-}" in
  MINGW*|MSYS|UCRT*|CLANG*)
    command -v cygpath >/dev/null || fail "cygpath is required in Git Bash."
    GIT_BASH=1
    [[ "${MSYS_NO_PATHCONV:-}" == "1" ]] ||
      fail "In Git Bash, run 'export MSYS_NO_PATHCONV=1' in this terminal before running the wrapper."
    ;;
esac
...
ANALYZER_PATH="$SCRIPT_DIR/analyze.py"
REPORT_PATH="$HTML"
if [[ "$GIT_BASH" -eq 1 ]]; then
  ANALYZER_PATH="$(cygpath -am "$ANALYZER_PATH")" || fail "Could not convert the analyzer path."
  REPORT_PATH="$(cygpath -am "$REPORT_PATH")" || fail "Could not convert the HTML report path."
fi

RC=0
python3 "$ANALYZER_PATH" "$SOURCE_KIND" "$SOURCE_ID" \
  --endpoint "$ENDPOINT" --port "$PORT" "${SUB_ARGS[@]}" --html "$REPORT_PATH" || RC=$?
