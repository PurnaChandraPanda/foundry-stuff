...
class CliTests(unittest.TestCase):
    ...
    def test_azure_stderr_and_invalid_json_are_not_absence(self):
        for stdout, stderr, status in (
            ...
            with self.subTest(stdout=stdout, status=status):
                with patch.object(analyzer.shutil, "which", return_value="az"), patch.object(
                    analyzer.subprocess,
                    "run",
    ...
    def test_subprocess_failure_exits_two_without_traceback(self):
        stdout, stderr = io.StringIO(), io.StringIO()
        with (
            patch.object(analyzer.shutil, "which", return_value="az"),
            patch.object(
                sys,
                "argv",
                [
    ...
    def test_missing_az_and_timeout_are_setup_errors(self):
        for error in (FileNotFoundError("az"), subprocess.TimeoutExpired(["az"], 120)):
            with self.subTest(error=error):
                with (
                    patch.object(analyzer.shutil, "which", return_value="az"),
                    patch.object(analyzer.subprocess, "run", side_effect=error),
                ):
                    with self.assertRaises(analyzer.AzError):
                        analyzer.az(["account", "show"])

    def test_azure_cli_uses_resolved_launcher(self):
        for executable in (r"C:\Program Files\Azure CLI\az.cmd", "/usr/bin/az"):
            with (
                self.subTest(executable=executable),
                patch.object(analyzer.shutil, "which", return_value=executable) as which,
                patch.object(
                    analyzer.subprocess,
                    "run",
                    return_value=subprocess.CompletedProcess(
                        [executable], 0, '{"id": "test"}', ""
                    ),
                ) as run,
            ):
                self.assertEqual(analyzer.az(["account", "show"], sub="test"), {"id": "test"})
                which.assert_called_once_with("az")
                run.assert_called_once_with(
                    [executable, "account", "show", "-o", "json", "--subscription", "test"],
                    capture_output=True,
                    text=True,
                    timeout=120,
                )

    def test_missing_azure_cli_on_path_does_not_launch(self):
        with (
            patch.object(analyzer.shutil, "which", return_value=None),
            patch.object(analyzer.subprocess, "run") as run,
        ):
            with self.assertRaisesRegex(analyzer.AzError, "az CLI not found on PATH"):
                analyzer.az(["account", "show"])
