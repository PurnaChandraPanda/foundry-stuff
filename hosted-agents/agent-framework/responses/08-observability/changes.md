...
- In local, navigate to directory - `hosted-agents/agent-framework/responses/08-observability`.

- Navigate to env file: `src/agent-framework-agent-observability-responses/.env`. Update the enviornment key-value pair details.
...
```
python src/agent-framework-agent-observability-responses/main.py
```
...
set -a
source src/agent-framework-agent-observability-responses/.env
set +a

# Run azd init
azd ai agent init \
  --no-prompt --force \
  --agent-name agent-framework-agent-observability-responses \
  -e obs-agent-dev \
  --project-id "$FOUNDRY_PROJECT_ID" \
  --model-deployment "$AZURE_AI_MODEL_DEPLOYMENT_NAME" \
  --src ./src/agent-framework-agent-observability-responses \
  --deploy-mode code \
  --runtime python_3_13 \
  --entry-point main.py
...
Get-Content .\src\agent-framework-agent-observability-responses\.env | ForEach-Object {
    if ($_ -match '^\s*([^#][^=]+?)\s*=\s*(.*)\s*$') {
        $name = $matches[1].Trim()
...
- Add the extra required environment variables for azure.yaml - to get observability sorted.
...
Run PowerShell as administrator and enable it at windows registry level.
...
azd ai agent show agent-framework-agent-observability-responses

# test the deployed agent
azd ai agent invoke agent-framework-agent-observability-responses "Hi"
