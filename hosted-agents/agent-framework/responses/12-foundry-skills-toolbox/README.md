...
An [Agent Framework](https://github.com/microsoft/agent-framework) agent that loads its behavioral guidelines from [**Foundry Skills**](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/tools/skills?view=foundry&pivots=python), hosted using the **Responses protocol**. Skills are authored once as `SKILL.md` files and uploaded to your Foundry project through `AIProjectClient.beta.skills`. Local runs can download the current versions from Foundry, while hosted deployments use the packaged copies so the readiness endpoint is not delayed by startup network calls.
...
### Loading skills at agent startup

[`main.py`](src/agent-framework-agent-foundry-skills-responses/main.py) reads the comma-separated `SKILL_NAMES` env var and the `SKILL_SOURCE` mode:

- `bundled` loads the requested `skills/<name>/SKILL.md` files included in the deployment package. This is the hosted default because it requires no network call before `/readiness` can return HTTP 200.
- `foundry` opens an `AIProjectClient`, downloads each requested skill through `beta.skills.download(name)`, and unpacks it into the writable operating-system temporary directory. This mode is useful locally when you want the latest Foundry versions without repackaging.

A [`SkillsProvider`](../../../../../packages/core/agent_framework/_skills.py) is built over the selected paths and attached to the `Agent` as a context provider. The provider follows the [Agent Skills](https://agentskills.io/) progressive-disclosure pattern:
...
This means the model only pays the token cost for a skill's full body when it actually needs it. In `foundry` mode, restarting picks up a newly provisioned version. In hosted `bundled` mode, redeploy after changing a skill.
...
In `foundry` mode, startup logs should include:
...
Downloaded `SKILL.md` files land under `foundry-skills/downloaded_skills/<name>/SKILL.md` in the operating system's temporary directory. The hosted configuration in `azure.yaml` sets `SKILL_SOURCE=bundled` to avoid this readiness-blocking download.
