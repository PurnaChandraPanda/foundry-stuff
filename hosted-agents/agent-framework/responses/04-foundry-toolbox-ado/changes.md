...
- Now, navigate into directory - `hosted-agents/agent-framework/responses/04-foundry-toolbox-ado`. 
...
curl -X POST http://localhost:8088/responses -H "Content-Type: application/json" -d '{ "input": "List recent 3 commits in the project name: wfm-proxy, repo name: wfm-proxy, branch name: master"}'
...
azd ai agent init \
  --no-prompt --force \
  --agent-name agent-framework-toolboxados-responses \
  -e "$AZD_ENV_NAME" \
...
azd ai agent show agent-framework-toolboxados-responses

# test the deployed agent
azd ai agent invoke agent-framework-toolboxados-responses "Hi"

azd ai agent invoke agent-framework-toolboxados-responses "what tools do you have"

azd ai agent invoke agent-framework-toolboxados-responses "what tools are there in the connected mcp"

azd ai agent invoke agent-framework-toolboxados-responses "List recent 3 commits in the project name: wfm-proxy, repo name: wfm-proxy, branch name: master"
