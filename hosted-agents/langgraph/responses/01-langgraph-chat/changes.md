...
- In local, navigate to directory - `hosted-agents/langgraph/responses/01-langgraph-chat`.

- Navigate to env file: `src/langgraph-chat-responses/.env`. Update the enviornment key-value pair details.
...
```
python src/langgraph-chat-responses/main.py
```

- Test the local agent hosted code to see if everything is fine

```
curl -X POST http://localhost:8088/responses -H "Content-Type: application/json" -d '{ "input": "what time is it right now" }'
```
...
set -a
source src/langgraph-chat-responses/.env
set +a

# Run azd init
azd ai agent init \
  --no-prompt --force \
  --agent-name langgraph-chat-responses \
  -e lgbasic-agent-dev \
  --project-id "$FOUNDRY_PROJECT_ID" \
  --model-deployment "$AZURE_AI_MODEL_DEPLOYMENT_NAME" \
  --src ./src/langgraph-chat-responses \
  --deploy-mode code \
  --runtime python_3_13 \
  --entry-point main.py
...
Get-Content .\src\langgraph-chat-responses\.env | ForEach-Object {
    if ($_ -match '^\s*([^#][^=]+?)\s*=\s*(.*)\s*$') {
        $name = $matches[1].Trim()
...
azd ai agent show langgraph-chat-responses

# test the deployed agent
azd ai agent invoke langgraph-chat-responses "what time is it right now"

azd ai agent invoke langgraph-chat-responses "what is 42*17"
azd ai agent invoke langgraph-chat-responses "add 100 to the result"
...
# test the deployed agent
azd ai agent invoke langgraph-chat-responses "Hi"
