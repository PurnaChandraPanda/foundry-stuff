...
- In local, navigate to directory - `hosted-agents/langgraph/responses/08-observability`.

- Navigate to env file: `src/langgraph-observability-responses/.env`. Update the enviornment key-value pair details.
...
```
python src/langgraph-observability-responses/main.py
```
...
set -a
source src/langgraph-observability-responses/.env
set +a

# Run azd init
azd ai agent init \
  --no-prompt --force \
  --agent-name langgraph-observability-responses \
  -e lgobs-agent-dev \
  --project-id "$FOUNDRY_PROJECT_ID" \
  --model-deployment "$AZURE_AI_MODEL_DEPLOYMENT_NAME" \
  --src ./src/langgraph-observability-responses \
  --deploy-mode code \
  --runtime python_3_13 \
  --entry-point main.py
...
Get-Content .\src\langgraph-observability-responses\.env | ForEach-Object {
    if ($_ -match '^\s*([^#][^=]+?)\s*=\s*(.*)\s*$') {
        $name = $matches[1].Trim()
...
environmentVariables:
  ...
  - name: OTEL_AUTO_CONFIGURE_AZURE_MONITOR
    value: 'true'
...
azd ai agent show langgraph-observability-responses

# test the deployed agent
azd ai agent invoke langgraph-observability-responses "What time is it right now, and what is 42 multiplied by 17?"
