...
| File | Role |
|---|---|
| `src/fabric-dataagent-responses/main.py` | The agent you ship. Zero diagnostics code. |
| `src/fabric-dataagent-responses/main_diag.py` | The same agent, plus every diagnostic hook wired in. |
| `src/fabric-dataagent-responses/_diagnostics.py` | The reusable diagnostics module. Does nothing until switched on. |
...
| Location in `main_diag.py` | Hook | Runs in the container? |
|---|---|---|
| `build_agent()` | `configure()`, `set_context()`, `start()` | **Yes** |
| `Agent(...)` | `middleware=diagnostics.agent_middlewares()` | **Yes** |
| `main()` | `emit()` boot marker, `install_asgi(server)` | **No - local only** |

With `dependencyResolution: remote_build` the hosting runtime **imports the
module and supplies its own launcher**, so `main()` never executes in the
container. That is why `configure()`/`start()` live in `build_agent()`, which the
runtime does call.

The consequence for this Responses sample: the **boot marker and the ASGI request
log only appear when you run locally**. Deployed, you still get the full probe
cycle and the per-run agent middleware, which is where the useful signal is. In
the Invocations sample the server object exists at module scope, so
`install_asgi` works there in both cases.

This asymmetry is not cosmetic - it is what made three earlier rounds of
diagnostics invisible in this sample.

The agent middleware reports per-run caller identity: the identity the Fabric
tool's On-Behalf-Of exchange is attempted for.
...
- `_diagnostics.py` is duplicated in this sample and in
  `hosted-agent-invocations/`. The two copies must stay byte-identical; drift
